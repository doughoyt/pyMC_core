"""
SX1276 LoRa Radio Driver for Raspberry Pi
Implements the LoRaRadio interface using the SX1276 library
"""
...
class SX1276Radio(LoRaRadio):
    """SX1276 LoRa Radio implementation for Raspberry Pi"""
    ...
    def __init__(
        ...
        if SX1276Radio._active_instance is not None:
            logger.warning("Another SX1276Radio instance is already active - cleaning it up first")
            try:
    ...
    def begin(self) -> bool:
        """Initialize the SX1276 radio module. Returns True if successful, False otherwise."""
        # Prevent double initialization
        if self._initialized:
            logger.debug("SX1276 radio already initialized, skipping")
            return True
        ...
        try:
            ...
            raise RuntimeError(f"Failed to initialize SX1276 radio: {e}") from e
    ...
    def set_tx_power(self, power: int) -> bool:
        ...
        def set_power():
            self.tx_power = power
            self.lora.setTxPower(power, self.lora.TX_POWER_SX1276)
...
def create_SX1276_radio(**kwargs) -> SX1276Radio:
    """Create and initialize an SX1276 radio instance"""
    radio = SX1276Radio(**kwargs)
    if radio.begin():
        return radio
    else:
        raise RuntimeError("Failed to initialize SX1276 radio")
